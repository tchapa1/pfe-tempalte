...
@Component({
  selector: 'app-dgcreateprojet',
  templateUrl: './dgcreateprojet.component.html',
  styleUrls: ['./dgcreateprojet.component.css']
})
export class DgcreateprojetComponent implements OnInit {
      ...
      return this.apiService.createProjet(this.projetForm.value).subscribe({
        complete: () => {
          console.log('projet successfully created!'),
            this.ngZone.run(() => this.router.navigateByUrl('/dglistprojet'));
        },
        error: (e) => {
          console.log(e);
        },
