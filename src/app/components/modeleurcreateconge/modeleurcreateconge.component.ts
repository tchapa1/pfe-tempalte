...
@Component({
  selector: 'app-modeleurcreateconge',
  templateUrl: './modeleurcreateconge.component.html',
  styleUrls: ['./modeleurcreateconge.component.css']
})
export class ModeleurcreatecongeComponent implements OnInit {
  user:any = [];
  submitted = false;
  congeForm: FormGroup;
      ...
      return this.apiService.createConge(this.congeForm.value).subscribe({
        complete: () => {
          console.log('conge successfully created!'),
            this.ngZone.run(() => this.router.navigateByUrl('/modeleurlistconge'));
        },
        error: (e) => {
          console.log(e);
        },
