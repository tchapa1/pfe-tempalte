...
@Component({
  selector: 'app-dgcreateabsence',
  templateUrl: './dgcreateabsence.component.html',
  styleUrls: ['./dgcreateabsence.component.css']
})
export class DgcreateabsenceComponent implements OnInit {
      ...
      return this.apiService.createAbsence(this.absenceForm.value).subscribe({
        complete: () => {
          console.log('absence successfully created!'),
            this.ngZone.run(() => this.router.navigateByUrl('/dglistabsence'));
        },
        error: (e) => {
          console.log(e);
        },
